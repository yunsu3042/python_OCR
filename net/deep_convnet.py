...
"""정확도 99% 이상의 고정밀 합성곱 신경망
   ...
   conv - batchnorm - relu - conv - batchnorm - relu - pool -
   conv - batchnorm - relu - conv - batchnorm - relu - pool -
   affine - batchnorm - relu - dropout - affine - batchnorm - dropout - softmax"""
...
class DeepConvNet:
    def __init__(self, input_dim=(1,28,28), file_path=None,
        ...
        self.layers.append(BatchNormalization(gamma=1, beta=0))
    ...
    def gradient(self, x, t):
        ...
        for idx, layer_idx in enumerate([1, 4, 8, 11, 15, 18, 22, 26]):
            grads['G' + str(idx + 1)] = self.layers[layer_idx].dgamma
            grads['B' + str(idx + 1)] = self.layers[layer_idx].dbeta
            
        for idx, layer_idx in enumerate([0, 3, 7, 10, 14, 17, 21, 25]):
